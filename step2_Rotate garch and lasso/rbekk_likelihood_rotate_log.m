function [L,log_likelihood, RH] = rbekk_likelihood_rotate_log(theta, Y, X)
...
H = zeros(K,K,T);
logRH=zeros(K,K,T);
logRH(:,:,1)=P; % we set the unconditional covariance matrix of daily(close to close) return as starting point of revolved conditional covariance
log_likelihood = zeros(T,1);
I = eye(K);
for t=2:T
  logRH(:,:,t) = (I-B*B'-A*A') + B*logRH(:,:,t-1)*B'+ A*X(:,:,t-1)*A' ; %conditional covariance MATRIX, insure positive semidefinite
end
