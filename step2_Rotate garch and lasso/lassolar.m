...
%   indices = (1:p)';
 ...
 % for entire path
    q = info.steps + 1;
    sigma2e = sum((y - X*b0).^2)/(n-p); % MSE for OLS estimate
    ...
    for step = 1:q
      ...
      info.rss(step) = sum(r.^2); % residual sum-of-squares
      info.mse(step) = info.rss(step)/(n-length(A));  % MSE for lasso
      info.df(step) = length(A);
      info.Cp(step) = info.rss(step)/sigma2e - n + 2*info.df(step);
      info.AIC(step) = info.rss(step) + 2*sigma2e*info.df(step);
      info.BIC(step) = info.rss(step) + log(n)*sigma2e*info.df(step);
