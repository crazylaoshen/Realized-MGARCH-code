...
[theta_hat, log_likelihood] = fmincon(@(theta)rbekk_likelihood_diagonal(theta,lret,RK), theta, [],[],[],[],LB,UB,[],options); % constraint on rbekk
warning('on') %#ok<*WNON>
...
C(ind)=vechC;
C = C*C'
[L, H] = rbekk_likelihood_diagonal(theta_hat,  lret, RK);
L = -L;
