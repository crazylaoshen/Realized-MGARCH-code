...
%   BETA = LAR(X, Y, STOP, STOREPATH) with STOREPATH set to true will
...
%   BETA = LAR(X, Y, STOP, STOREPATH, VERBOSE) with VERBOSE set to true
...
%       info.df(step) = step - 1;
      info.Cp(step) = rss/sigma2e - n + 2*length(A);
      info.AIC(step) = rss + 2*sigma2e*info.length(A);
      info.BIC(step) = rss + log(n)*sigma2e*length(A);
